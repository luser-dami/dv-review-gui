...
- **待提交变更**：A/M/D/R 徽章 + 每文件 +/− 统计，点击即看 diff（客户端模型即时切换，零网络）
- **工作区 diff 模型同步**：服务端缓存 diff 模型 + 客户端镜像；进入视图加载一次，
  之后行级回退直接改写双方模型（绕开 dv 秒级同步滞后），校验不一致自动强制重载
- **行级回退**：悬停变更行 → `↩` 直接改工作区文件（严格行内容校验、CRLF 保留、拒绝越界路径）
- **Review 全流程**：发起（当前分支或任意分支对）、变更文件列表、逐文件 / 完整 diff（语法高亮）、行内与整篇评论、批准 / 请求修改 / 拒绝、合并、关闭
- **键盘**：`R` 刷新（状态 + diff 模型）、文件行可 Tab 聚焦 + Enter 打开
